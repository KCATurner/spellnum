...
X_LEXICAL_EXCEPTION = re.compile(r'(?<=^se)(?=[co])')
S_LEXICAL_EXCEPTION = re.compile(r'(?<=^se)(?=[qtv])|(?<=^tre)(?=[coqtv])')
M_LEXICAL_EXCEPTION = re.compile(r'(?<=^septe)(?=[ov])|(?<=^nove)(?=[ov])')
N_LEXICAL_EXCEPTION = re.compile(r'(?<=^septe)(?=[cdqst])|(?<=^nove)(?=[cdqst])')

VALID_NUMERIC_FLOAT = re.compile((r'^(?P<sign>[-+]?)(?# capture sign if exists)'
