...
VALID_INPUT_NUMBER = _re.compile((r'^(?P<sign>[-+])?(?# capture sign if exists)'
...
VALID_INPUT_TEXT = _re.compile((r'^(?:(?P<whole>.+)\s+and\s+)?(?# " and " separates whole from fraction)'
                                r'(?P<numerator>.+\s+)?(?# fraction made of numerator and base-10 denominator)'
                                r'(?P<denominator>(?:one hundred|ten|one\s+)(?# denominator period value)'
