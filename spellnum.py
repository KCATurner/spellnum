...
def _period_name(baseillion):
    """
    Constructs and the period name from a three dimensional table of prefixes for each digit in the given base-illion
    ...
    if baseillion < 0: return ''
    if baseillion < 10: return UNIQUE_PERIODS[baseillion]
    ...
    if not str(unit_prefix).endswith('e'): return result.replace("allion", "illion")
    if 'sec' in result or 'seo' in result: unit_prefix += 'x'
    if 'seq' in result or 'set' in result or 'sev' in result: unit_prefix += 's'
    if 'trec' in result or 'treo' in result or 'treq' in result or 'tret' in result or 'trev' in result: unit_prefix += 's'
    if 'septeo' in result or 'septev' in result: unit_prefix += 'm'
    if 'septec' in result or 'septed' in result or 'septeq' in result or 'septes' in result or 'septet' in result: unit_prefix += 'n'
    if 'noveo' in result or 'novev' in result: unit_prefix += 'm'
    if 'novec' in result or 'noved' in result or 'noveq' in result or 'noves' in result or 'novet' in result: unit_prefix += 'n'
...
def _spell_period(period):
    ...
    if tens < 20: suffix = NUMB_LT_TWENTY[tens + unit]
...
def spell_integer(number):
    ...
    for period in periods:
        name = _period_name(baseillion=baseillion)
